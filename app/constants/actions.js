/**
 * @file app/constants/actions.js
 * Action constants are defined here
 */

export const APP_PAGE_CHANGED = 'a_apc';
export const APP_WINDOW_RESIZED = 'a_awr';

export const BROWSER_LIST_ARTISTS_REQUESTED     = 'a_bla';
export const BROWSER_LIST_ALBUMS_REQUESTED      = 'a_blb';
export const BROWSER_ARTIST_LIST_ITEM_SELECTED  = 'a_bals';
export const BROWSER_TRACK_LIST_ITEM_SELECTED   = 'a_btls';
export const BROWSER_TRACK_LIST_ITEM_PLAYED     = 'a_btlp';
export const BROWSER_SONGS_LIST_RECEIVED        = 'a_bslr';
export const BROWSER_NEXT_SECTION_SWITCHED_TO   = 'a_bnss';

